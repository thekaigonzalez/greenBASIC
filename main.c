...
#define MAX_STATS                                                             \
  10 /* 10 statements. blunt but like i cant do nothing about it for now */
#define MAX_ARGS                                                              \
  5 /* max 5 arguments -> very blunt but my bedtime is soon and im trying to  \
       complete this in 2 hours and ive wasted too much time */

#define S_IDLE                                                                \
  -1 /* instead of obviously using more concentrated enum, we use defines     \
        with numbers */
#define S_IGNORANT                                                            \
  0 /* basically strings and anything that ignores all of the other states */
#define S_TNAME                                                               \
  1 /* essentially gathering a type name / starter (PRINT "hello"); PRINT     \
       being the type name */
#define S_TPM 2 /* TPM -> T-ype P-ara M-eters*/

#define BA_EOL ';'   /* semicolon to end statement */
#define BA_SPACE ' ' /* space to separate params and func */
#define BA_EOF '\0'  /* End Of File */

#define basic_error(x)                                                        \
  printf ("greenBASIC: error: %s\n", x);                                      \
  exit (-1);
...
void
BASIC_SetState (BAZBuff *__buf, int stat)
{
  __buf->state = stat;
}

int
PRINT_FUNCTION (char **ARGV)
{
  printf ("%s\n", *ARGV);
}

void
run_greenBASIC (const char *__n)
{
  ...
  struct gb_func_t NAMES[] = { { "PRINT", PRINT_FUNCTION } };
  ...
  if (_tmp.state == S_IGNORANT)
    basic_error ("failed to execute statement due to unfinished quote");
}

main () { run_greenBASIC ("PRINT \"hello world 1\";"); }
